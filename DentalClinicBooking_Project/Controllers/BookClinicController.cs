...
namespace DentalClinicBooking_Project.Controllers
{
    public class BookClinicController : Controller
    {
        ...
        public BookClinicController(DentalClinicBookingProjectContext context, IClinicRepository bookClinicRepository,
        ...
        public async Task<IActionResult> AppointmentBookingInfo([FromBody] AppointmentBookingViewModel appointmentBookingModel)
        {
            ...
            if (!string.IsNullOrEmpty(patientString))
            {
                ...
                if (model == null)
                {
                    ...
                    _contx.HttpContext.Session.SetString("clinicAppointmentSchedule", JsonConvert.SerializeObject(clinicAppointmentSchedule, settings));
        ...
        {
            var clinicAppointmentScheduleString = _contx.HttpContext?.Session.GetString("clinicAppointmentSchedule");
            var clinicAppointmentSchedule = JsonConvert.DeserializeObject<ClinicAppointmentSchedule>(clinicAppointmentScheduleString);
            string serviceName = _context.Services
                        .Where(x => x.ServiceId.Equals(clinicAppointmentSchedule.ServiceId))
            ...
            itemList.items.Add(new Item()
            {
                name = serviceName,
                currency = "USD",
                price = Math.Round(clinicAppointmentSchedule.Price / 23500, 2).ToString(),
                quantity = "1",
                sku = clinicAppointmentSchedule.ServiceId.ToString(),
            });
            var payer = new Payer()
            {
            ...
            var details = new Details()
            {
                tax = "0",
                shipping = "0",
                subtotal = Math.Round(clinicAppointmentSchedule.Price / 23500, 2).ToString(),
            };
            //Final amount with details  
            var amount = new Amount()
            {
                currency = "USD",
                total = Math.Round(clinicAppointmentSchedule.Price / 23500, 2).ToString(), // Total must be equal to sum of tax, shipping and subtotal.  
                details = details
            };
